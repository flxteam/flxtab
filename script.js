// --- Supabase & State Initialization ---
        ...
        async function fetchSuggestions(query) {
            const currentEngineName = document.querySelector('.custom-select-trigger span').textContent;
            const apiProvider = suggestionApis[currentEngineName];
            ...
            try {
                let suggestions = [];
                if (currentEngineName === 'Baidu') {
                    // Baidu API 返回 JSONP，需要特殊处理
                    const response = await fetchJsonp(apiProvider(query));
                    if (response && response.s) {
                        suggestions = response.s;
                    }
                } else if (currentEngineName === 'Bing') {
                    const response = await fetch(apiProvider(query));
                    const data = await response.json();
                    if (data && data.AS && data.AS.Results && data.AS.Results.length > 0) {
                        suggestions = data.AS.Results[0].Suggests.map(s => s.Txt);
                    }
                } else if (currentEngineName === 'Google') {
                    const response = await fetch(apiProvider(query));
                    const text = await response.text();
                    // Google API 返回 XML，需要解析
                    const parser = new DOMParser();
                    const xmlDoc = parser.parseFromString(text, 'application/xml');
                    const suggestionNodes = xmlDoc.querySelectorAll('suggestion');
        ...
        // 辅助函数：处理 JSONP 请求 (Baidu)
        function fetchJsonp(url) {
            return new Promise((resolve, reject) => {
                const script = document.createElement('script');
                const callbackName = 'jsonp_callback_' + Math.round(100000 * Math.random());
                url += (url.indexOf('?') >= 0 ? '&' : '?') + 'cb=' + callbackName;
                script.src = url;
        ...
        // --- Initializations and Event Listeners (Moved to the end for clarity) ---
            ...
            deleteButton.addEventListener('click', deleteQuickLink);
            linkWrapper.appendChild(linkElement);
            linkWrapper.appendChild(deleteButton);
            const addButton = quickLinksContainer.querySelector('.add-button');
            if (addButton) {
                quickLinksContainer.insertBefore(linkWrapper, addButton);
            } else {
                quickLinksContainer.appendChild(linkWrapper);
            }
        }
